...
- 답변은 한국어로 하되 필수적인 부분에서는 영어를 사용한다.
...
기획 터미널(기획자 에이전트)은 사용자가 진행 상황을 물으면 `current-task.md`의 "상태"를 읽고 다음 형식으로 한 줄 보고한다.

- 정상 진행 중: "현재 단계: (예: 보안/설계 리뷰)"
- 검증 실패로 회귀한 경우: "검증 단계에서 오류 발생 → 설계 단계로 복귀 (실패 범위: ...)"

전체 진행 과정을 처음부터 재설명하지 않고, 현재 단계와(있다면) 회귀 사실만 간결히 전달한다.
