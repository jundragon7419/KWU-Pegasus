...
// poll row를 받아 옵션/투표자/공개여부를 포함한 응답 데이터를 구성
async function loadPollData(poll, userId, userRole) {
...
exports.submitVote = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // 새로운 투표 저장 및 투표 수 증가 (익명/기명 여부와 관계없이 userId를 저장)
      const voteValues = optionIds.map(optionId => [pollId, userId, optionId])
      await connection.query(
